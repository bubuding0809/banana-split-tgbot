...
class GroupCommandHandler(BaseHandler):
    ...
    async def _update_chat_thread(
        ...
        payload = UpdateChatPayload(chat_id=update.effective_chat.id, thread_id=thread_id)
    ...
    async def set_topic(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if message_thread_id is None or not ChatUtils.is_forum_chat(update.effective_chat):
            return await self.send_error_message(
    ...
    async def bot_added(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        bot = next(
            (member for member in new_members if member.username == context.bot.username),
            None
        )
